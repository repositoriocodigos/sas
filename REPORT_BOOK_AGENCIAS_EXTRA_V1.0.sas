...
PROC SORT/*ORDENANDO BASE*/
	...
	BY C_CODI P_TITULO C_MOTIVO_BXA DATA_ARQUIVO;
RUN;
...
PROC SORT/*VERIFICANDO DUPLICIDADE*/
	...
	DATA= REPORT.BAIXA_EX 
		NODUPKEY;
	BY C_CODI P_TITULO C_MOTIVO_BXA;
RUN;
...
PROC SQL;/* CRUZANDO INFO PART_1*/
	CREATE TABLE REPORT.BASE_7 AS SELECT
		*
	FROM REPORT.REMESSA_EX A 
		LEFT JOIN  REPORT.BAIXA_2 B ON A.C_CODI = B.C_CODI 
...
PROC SQL;
	CREATE TABLE REPORT.BASE_6_3 AS 
		SELECT 
			P_TITULO,
...
PROC SQL;
	CREATE TABLE REPORT.BASE_6_4 AS 
		SELECT    
			* 
		FROM REPORT.BASE_9  AS A LEFT JOIN REPORT.BASE_6_3 AS B 
...
PROC EXPORT DATA= REPORT.BASE_ESCOBS /*EXPORT DA BASE FINAL PARA CARREGAR O REPORT DAS AGENCIAS*/
	OUTFILE= "\\BRSLP1W8PFS03\GRUPOS\AFINIDADE\PLANEJAMENTO_CREDITO_COBRANCA\008_M.I.S\03_BOOK_ESCOBS\TXT\BASE_EXTRA.TXT"
		DBMS=TAB 
		REPLACE;
RUN;

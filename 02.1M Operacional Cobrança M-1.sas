...
/* INFORMAÇÃO:
MÊS DE REFERÊNCIA É O MÊS ANTERIOR 
...
%MACRO OpCobrancaM1(ANOMES,ANOMES_M1,ANOMES_1M,DTFIMMES,DTINMES);
...
proc sql;
	create table work.tmpDROP as
		select *
	from work.tmp (drop= VENC_TITULO idcliente) ;
QUIT;
...
SELECT
	...
	(case
		...
		when ((&DTFIMMES - t1.DATA_SUSPENSAO) >=60 and (t1.DATA_SUSPENSAO <= t1.DATA_ARQUIVO)and (t1.DATA_ARQUIVO  - t1.VENC_TITULO ) > 30) then 'Fase 3'
		else 'Fase 2'
	END) as FaseCob,
...
proc append base=work.REMESSA data=bdescobs.BDESCOB_&ANOMES_1M FORCE; /* CAPTURANDO INFO DE BAIXA MES REF +1 */          
run;
...
PROC SQL; 
CREATE TABLE workbck.PC as /* 1.3 CAPTURANDO INFO DE PRESTCAO MES REF.*/
    select *
    from BDPRESTA.BDPRESTACAO_&ANOMES;
RUN;


proc append base=workbck.PC data=BDPRESTA.BDPRESTACAO_&ANOMES_M1 FORCE; /* CAPTURANDO INFO DE PRESTCAO MES REF. -1*/          
run;

proc append base=workbck.PC data=BDPRESTA.BDPRESTACAO_&ANOMES_1M FORCE; /* CAPTURANDO INFO DE PRESTCAO MES REF. +1*/           
run;
...
PROC SQL;/* CRUZANDO INFO PART_1 - BAIXA*/
    ...
    FROM workbck.goon3 A LEFT JOIN  workbck.BAIXA B ON A.C_ID_EMP = B.C_ID_EMP;
RUN;
...
PROC SQL;/* CRUZANDO INFO PART_2 - PRESTACAO*/
    ...
    FROM workbck.goon3_0 A LEFT JOIN  workbck.PC B ON A.C_ID_EMP = B.P_ID_EMP;
RUN;

DATA workbck.PGTs;/*  CAPTURANDO INFO DE PGT */
    SET    
  		GOONPGT.PGT_&ANOMES.S 
        GOONPGT.PGT_&ANOMES_1M.S
...
PROC SQL;  /* REVER O ROLOU PARA O CASO DE m-1 */
...
/* Group By data de enviou, Fase de Cobrança, Atraso e Empresa de Cobrança & somanto os Valores e Quantidade	
*/
...
proc export data=workbck.goonFS1_&ANOMES 
	outfile = "C:\Users\Di Giunta\Documents\GoOn\Nextel\01 Planilha\02 Operacional Cobrança\Operacional Fase1_&ANOMES. M-1.csv"
	dbms=csv
	replace;
run;
...
proc export data=workbck.goonFS2_3_&ANOMES 
	outfile = "C:\Users\Di Giunta\Documents\GoOn\Nextel\01 Planilha\02 Operacional Cobrança\Operacional Fase2 e 3_&ANOMES. M-1.csv"
	dbms=csv
	replace;
run;
...
%OpCobrancaM1(201504, 201503, 201505,'31Apr2015'd, '01Apr2015'd);
